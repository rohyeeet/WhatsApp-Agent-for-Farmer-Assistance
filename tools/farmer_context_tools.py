...
from tools.memory_tools import get_farmer_profile, get_farmer_insights

def load_farmer_profile(user_id: str = 'default_user'):
    ...
    try:
        ...
        insights = get_farmer_insights(user_id, last_n=15)
        return {
            'status': 'success',
            'farmer_profile': profile_data['farmer_details'],
            'past_insights': insights,
...
def get_farmer_context_summary(user_id: str = 'default_user'):
    ...
    insights = profile_result.get('past_insights', [])
    summary = {
        'status': 'success',
        'user_id': user_id,
        'farmer_summary': {
        ...
        'past_conversation_insights': insights
    }
...
def get_crop_specific_context(crop_name: str, user_id: str = 'default_user'):
...
def get_seasonal_recommendations(user_id: str = 'default_user'):
